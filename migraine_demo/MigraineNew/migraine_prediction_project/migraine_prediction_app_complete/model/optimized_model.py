...
class OptimizedMigrainePredictionModel:
    ...
    def focal_loss(self, gamma=2.0, alpha=0.25):
        ...
        def focal_loss_fn(y_true, y_pred):
            # --- Numerical Stability --- 
            epsilon = tf.keras.backend.epsilon() # Get default epsilon (usually 1e-7)
            y_pred = tf.clip_by_value(y_pred, epsilon, 1. - epsilon) # Clip probabilities
            # --- End Stability ---

            # Convert to logits if needed (We assume y_pred is probability from Sigmoid now)
            # No need to check op type if we always clip and convert
            y_pred_logit = tf.math.log(y_pred / (1. - y_pred)) # Calculate logits safely

            # Calculate binary cross entropy from logits
            bce = tf.keras.losses.BinaryCrossentropy(
                from_logits=True, 
                reduction=tf.keras.losses.Reduction.NONE
            )(y_true, y_pred_logit) # Use safe logits
    ...
    def train_optimized_model(self):
        ...
        # --- Add Logging Here ---
        print("--- Shapes and Dtypes before model.fit --- ")
        try:
            print(f"y_train shape: {y_train.shape}, dtype: {y_train.dtype}")
            print(f"X_train_list shapes: {[x.shape for x in X_train_list]}")
            print(f"X_train_list dtypes: {[x.dtype for x in X_train_list]}")
            print(f"y_val shape: {y_val.shape}, dtype: {y_val.dtype}")
            print(f"X_val_list shapes: {[x.shape for x in X_val_list]}")
            print(f"X_val_list dtypes: {[x.dtype for x in X_val_list]}")
        except Exception as e:
            print(f"Error printing shapes/dtypes: {e}")
        print("--- End Shapes and Dtypes --- ")
        # --- End Logging ---
    ...
    def optimize_high_risk_threshold(self, model, X_val_list, y_val):
        ...
        for threshold in tqdm(thresholds, desc="Optimizing Threshold"):
            y_pred = tf.cast(y_pred_prob >= threshold, dtype=tf.int32)
            tn, fp, fn, tp = confusion_matrix(y_val, y_pred.numpy()).ravel()
    ...
    def run_optimization(self):
        ...
        if not final_metrics['overall_target_met']:
            ...
            ensemble_seeds = [100, 200, 300] # Define seeds
            for seed in tqdm(ensemble_seeds, desc="Ensemble Training"):  # Wrap iterable with tqdm
