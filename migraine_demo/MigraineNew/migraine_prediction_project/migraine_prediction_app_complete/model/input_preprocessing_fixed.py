"""
Model Input Preprocessing Module - Updated for 4-expert compatibility
...
def preprocess_expert_inputs(sleep_data, weather_data, stress_diet_data, physio_data=None):
    """
    ...
    Args:
        ...
        physio_data (array, optional): Physiological data with shape (samples, features)
    ...
    if physio_data is not None:
        if not isinstance(physio_data, tf.Tensor):
            physio_data = tf.convert_to_tensor(physio_data, dtype=tf.float32)
        expert_inputs = [sleep_data, weather_data, stress_diet_data, physio_data]
    else:
...
def format_input_for_prediction(X_test_list):
    ...
    This function has been updated to handle both 3-expert and 4-expert inputs.
    For 3-expert models, it uses sleep, weather, and stress/diet data.
    For 4-expert models, it also includes physiological data.
    
    Args:
        X_test_list (list): List of test data arrays for each expert
            [sleep_data, weather_data, stress_diet_data] or
    ...
    # Handle both 3-expert and 4-expert inputs
    if len(X_test_list) == 3:
        # Original 3-expert model
    ...
    elif len(X_test_list) == 4:
        # For 4-expert model, we need to adapt to the 3-expert model format
        # Extract the first 3 experts only for compatibility with original model
        sleep_data = X_test_list[0]  # Shape: (samples, seq_len, features)
        weather_data = X_test_list[1]  # Shape: (samples, features)
        stress_diet_data = X_test_list[2]  # Shape: (samples, seq_len, features)
        ...
        raise ValueError(f"Expected 3 or 4 arrays, got {len(X_test_list)}")
